...
(function ($, undefined) {
	...
	$(function () {
		...
		function formatName(val, obj) {
			return ['<a href="index.php?controller=pjAdmin&action=pjActionRedirect&nextController=pjAdminGroup&nextAction=pjActionView&calendar_id=', obj.id, '&nextParams=', encodeURIComponent("id=" + obj.id), '">', val, '</a>'].join("");
		}
		...
		if ($("#grid").length > 0 && datagrid) {
			...
			var gridOpts = {
				buttons: [ {type: "edit", title: myLabel.edit, url: "index.php?controller=pjAdmin&action=pjActionRedirect&nextController=pjAdminGroup&nextAction=pjActionView&calendar_id={:id}&nextParams=id%3D{:id}"},
