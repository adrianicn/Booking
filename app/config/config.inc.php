<?php
if (!defined("PJ_HOST")) define("PJ_HOST", "localhost");
if (!defined("PJ_USER")) define("PJ_USER", "root");
if (!defined("PJ_PASS")) define("PJ_PASS", "root");
if (!defined("PJ_DB")) define("PJ_DB", "igtrip");
if (!defined("PJ_PREFIX")) define("PJ_PREFIX", "booking_");
if (!defined("PJ_INSTALL_FOLDER")) define("PJ_INSTALL_FOLDER", "/Booking/");
if (!defined("PJ_INSTALL_PATH")) define("PJ_INSTALL_PATH", "/Applications/MAMP/htdocs/Booking/");
if (!defined("PJ_INSTALL_URL")) define("PJ_INSTALL_URL", "http://localhost/Booking/");
if (!defined("PJ_SALT")) define("PJ_SALT", "7EPQTJTO");
if (!defined("PJ_INSTALLATION")) define("PJ_INSTALLATION", "");
?>